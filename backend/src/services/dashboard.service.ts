...
export async function getSalesManagerStats() {
  ...
  const [totalLeads, convertedThisMonth, allLeads, salesAgents] = await Promise.all([
    Lead.countDocuments({}),
    Lead.countDocuments({
      "system.leadStatus": "Converted",
  ...
  return {
    ...
    stats: {
      ...
      monthlyConversions: convertedThisMonth,
      revenuePipeline: 0, // No deal value field yet
      pipeline: statusCounts,
...
export async function getBusinessHeadStats() {
  const startOfMonth = new Date();
  startOfMonth.setDate(1);
  startOfMonth.setHours(0, 0, 0, 0);
...
export async function getDashboardStats(userId: string, role: string) {
  ...
  switch (normalizedRole) {
    ...
    case "sales_manager":
      return getSalesManagerStats();
    case "business_head":
      return getBusinessHeadStats();
    case "admin":
      return getAdminStats();
    default:
      return getBusinessHeadStats(); // Default to business head view
  }
}
